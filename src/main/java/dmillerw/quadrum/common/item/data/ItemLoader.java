...
public class ItemLoader {
   ...
   public static void initialize() {
      for (File file : Quadrum.itemDir.listFiles(new ExtensionFilter("json"))) {
         try {
            ...
            if (jsonElement != null) {
               if (jsonElement.isJsonArray()) {
                  ...
                  while (iterator.hasNext()) {
                     ...
                     if (element != null && element.isJsonObject() && JsonVerification.verifyRequirements(file, element.getAsJsonObject(), ItemData.class)) {
            ...
            Quadrum.log(Level.WARN, "Completely failed to generate item from %s. Reason: %s", file.getName(), var7.toString());
         }
      }
   }
   ...
   private static void parse(String filename, JsonObject jsonObject) {
      try {
         ...
         for (Entry<String, Float> entry : itemData.mobDrops.entrySet()) {
