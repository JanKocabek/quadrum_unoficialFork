...
public class ItemBlockQuadrum extends ItemBlock {
   ...
   public ItemBlockQuadrum(Block block) {
      ...
      if (block instanceof IQuadrumBlock) {
         this.name = ((IQuadrumBlock)block).getName();
         this.setMaxStackSize(BlockLoader.blockDataMap.get(this.name).maxStackSize);
      } else {
   ...
   public boolean placeBlockAt(
      ...
      if (BlockLoader.blockDataMap.get(this.name).getBlockType() == TypeSpecific.Type.BLOCK && result) {
         int l = MathHelper.floor_double(player.rotationYaw * 4.0F / 360.0F + 0.5) & 3;
         if (l == 0) {
